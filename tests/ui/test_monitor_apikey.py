...
def test_poll_api_key_router_uses_store(qtbot, monkeypatch):
    ...
    qtbot.addWidget(w)
    monkeypatch.setattr(
        mw.api_key_store, "read_api_key", lambda base, name: "sk-router"
    )
...
def test_refresh_props_sends_server_api_key(qtbot, monkeypatch):
    ...
    w = mw.MainWindow()
    qtbot.addWidget(w)
    w._monitor._props = None
    w._monitor._props_model = None
...
def test_collect_monitor_data_sends_server_api_key(qtbot, monkeypatch):
    ...
    w._configure_panel.load_profile(
        _server(**{"api-key": "sk-secret", "metrics": True})
    )
...
def test_monitored_router_instance_resolves_router_key(qtbot, tmp_path, monkeypatch):
    ...
    w._monitor._active_instance = Instance(
        name="llama-gemma",
        profile="Gemma",
        mode="router",
        running=True,
...
def test_metrics_report_sends_key_and_model_scope_for_router(
