...
def _track_terminal(monkeypatch):
    launched = []
    monkeypatch.setattr(
        lc.terminal, "launch", lambda argv, template=None: launched.append(argv)
    )
    return launched
...
def test_preset_warning_abort_stops_router_launch(main_window, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        main_window,
        "prepare_router_files",
        lambda: (str(tmp_path), ["preset x is stale"]),
    )
    built = []
    monkeypatch.setattr(
        lc, "build_command", lambda p, **kw: built.append(p) or ["podman"]
    )
    _answer_warning(monkeypatch, QMessageBox.Abort)
...
def test_preset_warning_ignore_proceeds_with_router_launch(
