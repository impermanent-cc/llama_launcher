...
def test_add_docker_node_threads_binary_to_context_create(tmp_path, qtbot):
    ...
    from llama_launcher.ui.dialogs.nodes_dialog import NodesDialog
    from llama_launcher.store.nodes import load_nodes
    dlg = NodesDialog(tmp_path); qtbot.addWidget(dlg)
    seen = {}
    with patch("llama_launcher.ui.dialogs.nodes_dialog.runtime.connection_add_argv",
               side_effect=lambda name, ssh, binary: seen.setdefault("binary", binary) or ["x"]), \
         patch("llama_launcher.ui.dialogs.nodes_dialog._run_ok", return_value=True):
        dlg.add_node_from_fields("box-d", "me@10.0.0.2", "docker")
    assert seen["binary"] == "docker"
    node = [n for n in load_nodes(tmp_path) if n.name == "box-d"][0]
...
def test_remove_docker_node_threads_binary(tmp_path, qtbot):
    ...
    from llama_launcher.ui.dialogs.nodes_dialog import NodesDialog
    add_node(Node(name="box-d", kind="remote", connection="box-d",
                  ssh_target="me@10.0.0.2", binary="docker"), tmp_path)
    dlg = NodesDialog(tmp_path); qtbot.addWidget(dlg)
    row = next(i for i in range(dlg.list.count())
               if dlg.list.item(i).text().startswith("box-d"))
    dlg.list.setCurrentRow(row)
    seen = {}
    with patch("llama_launcher.ui.dialogs.nodes_dialog.runtime.connection_remove_argv",
               side_effect=lambda name, binary="podman": seen.setdefault("binary", binary) or ["x"]), \
         patch("llama_launcher.ui.dialogs.nodes_dialog._run_ok", return_value=True):
        dlg._on_remove()
    assert seen["binary"] == "docker"
