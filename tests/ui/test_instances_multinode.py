...
def _target(tmp_path):
    return {
        "binary": "podman", "base_dir": str(tmp_path),
        "router_base_dir": str(tmp_path),
        "nodes": [
            {"name": "local", "connection": "", "host": "127.0.0.1",
             "binary": "podman", "enabled": True},
            {"name": "box-b", "connection": "box-b", "host": "192.168.1.11",
             "binary": "podman", "enabled": True},
        ],
    }
...
def test_gather_tags_rows_by_node_and_survives_unreachable(tmp_path):
    ...
    with patch.object(mc.runtime, "list_launcher_containers", side_effect=fake_list), \
...
def test_row_carries_node_and_local_rows_use_node_local(tmp_path):
    with patch.object(mc.runtime, "list_launcher_containers",
                       return_value=[{"name": "llama-p", "running": True,
                                      "profile": "p", "mode": "server"}]), \
         patch.object(mc, "list_profiles", return_value=[]), \
         patch.object(mc.native, "list_native_instances", return_value=[]):
        data = mc.build_instances_data({
            "binary": "podman", "base_dir": str(tmp_path),
            "router_base_dir": str(tmp_path),
            "nodes": [{"name": "local", "connection": "", "host": "127.0.0.1",
                       "binary": "podman", "enabled": True}],
        })
...
def test_remote_row_renders_node_name_on_card(qtbot):
    c = StatCard("llama-p")
    qtbot.addWidget(c)
    c.update_row({"profile": "p", "port": 8080, "health": "ready", "tok_s": 12.0,
                  "kv_pct": 0.1, "embeddings": False, "reranking": False,
                  "mode": "server", "running": True, "node": "box-b"})
...
def test_local_row_card_title_has_no_node_suffix(qtbot):
    ...
    c.update_row({"profile": "p", "port": 8080, "health": "ready", "tok_s": 12.0,
                  "kv_pct": 0.1, "embeddings": False, "reranking": False,
                  "mode": "server", "running": True, "node": "local"})
