...
def test_fetch_button_has_tooltip(win):
    assert win._configure_panel.fetch_btn.toolTip().strip()
    # the tooltip must note the not-downloaded caveat
    assert "pull" in win._configure_panel.fetch_btn.toolTip().lower()
...
def test_fetch_button_relabeled(win):
    assert win._configure_panel.fetch_btn.text() == "Fetch latest"
...
def _ensure_no_benchmark(win):
    ...
    assert getattr(win._benchmark, "_benchmark_thread", None) is None
...
def test_update_badge_disabled_during_fetch_and_reenabled_on_finish(win, monkeypatch):
    ...
    win._launch._on_fetch_finished()
    assert win._configure_panel.update_badge.isEnabled() is True
