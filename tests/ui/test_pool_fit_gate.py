...
def _wire(main_window, monkeypatch, *, donations, estimate=10 * GIB, answer=None):
    ...
    monkeypatch.setattr(
        main_window._configure_panel, "_model_estimate_bytes", lambda p: estimate
    )
    ...
    if answer is not None:
        monkeypatch.setattr(
            lc.QMessageBox, "warning", staticmethod(lambda *a, **kw: answer)
        )
    return ctl, launched


def test_pool_launch_proceeds_when_fits(main_window, monkeypatch):
    ctl, launched = _wire(main_window, monkeypatch, donations=[("vram", 20 * GIB)])
    ctl.on_launch()
...
def test_pool_launch_probe_failure_fails_open(main_window, monkeypatch):
    ctl, launched = _wire(
        main_window, monkeypatch, donations=OSError("node unreachable")
    )
    ctl.on_launch()
    assert launched == ["pool"]
...
def test_pool_fit_dialog_shows_headline(main_window, monkeypatch):
    captured = []
    ctl, _launched = _wire(
        main_window, monkeypatch, donations=[("vram", 5 * GIB), ("ram", 2 * GIB)]
    )
    monkeypatch.setattr(
        lc.QMessageBox,
        "warning",
        staticmethod(
            lambda parent, title, text, *rest: (
