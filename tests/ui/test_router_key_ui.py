...
def test_switching_to_router_mode_populates_the_key(win):
    ...
    win._configure_panel._add_member_item(
        win._configure_panel._member_from_name("gen")
        if hasattr(win._configure_panel, "_member_from_name")
        else __import__(
            "llama_launcher.core.spec", fromlist=["RouterMember"]
        ).RouterMember(profile="gen")
    )
    win._configure_panel.mode_combo.setCurrentIndex(
        _router_index(win)
    )  # fires _on_mode_changed
    assert win._configure_panel.api_key_box._api_key, (
        "key should be generated on entering router mode"
    )
    win._configure_panel.api_key_box.reveal_check.setChecked(True)
    assert (
        win._configure_panel.api_key_box.key_label.text()
        == win._configure_panel.api_key_box._api_key
    )
...
def test_switching_to_configure_tab_populates_the_key(win):
    ...
    assert win._configure_panel.api_key_box._api_key, (
        "key should repopulate when the Configure tab is shown"
    )
...
def test_scope_radio_flows_into_current_profile(win):
    ...
    win._configure_panel.api_key_box.scope_own.setChecked(
        True
    )  # fires key_scope_changed
...
def test_saving_key_while_router_running_warns_a_relaunch_is_needed(win, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        mw.QMessageBox, "information", lambda *a, **k: calls.append((a, k))
    )
    win._configure_panel.api_key_box.set_scope("global")
...
def test_loading_profile_syncs_scope_radio(win):
    from llama_launcher.core.spec import RouterMember, Runtime
    ...
    p = Profile(
        ...
        image="img",
        mode="router",
        runtime=Runtime(router_key_mode="own"),
        settings={"port": 8080},
