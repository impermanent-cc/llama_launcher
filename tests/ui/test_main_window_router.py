...
def test_bind_host_round_trips(win):
    p = Profile(
        name="Host", mode="router", image="img", runtime=Runtime(bind_host="0.0.0.0")
    )
    win._configure_panel.load_profile(p)
...
def test_server_launch_still_uses_the_terminal(win, monkeypatch):
    ...
    monkeypatch.setattr(win._launch, "vram_check", lambda: None)
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.terminal.launch",
...
def test_detached_server_launch_uses_spawn_chain_not_terminal(win, monkeypatch):
    ...
    monkeypatch.setattr(
        "llama_launcher.ui.controllers.launch_controller.QMessageBox.critical",
        lambda *a, **k: None,
    )
...
def test_detached_server_error_dialog_survives_a_later_mode_switch(win, monkeypatch):
    ...
    assert dialogs, (
        "the dialog must still pop -- it was decided at launch "
...
def test_router_error_dialog_stays_off_after_a_later_mode_switch(win, monkeypatch):
    ...
    assert dialogs == [], (
        "the router launch's on_error must stay dialog-free "
...
def test_refresh_router_models_populates_the_panel(win, monkeypatch):
    ...
    win._configure_panel.load_profile(
        Profile(name="Host", mode="router", image="img", settings={"port": 8080})
    )
    monkeypatch.setattr(
...
def test_sleeping_models_are_not_polled_for_metrics(win, monkeypatch):
    ...
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.metrics.fetch_slots",
        lambda *a, **kw: calls.append(kw) or [],
    )
...
def test_loaded_model_is_polled_with_its_id(win, monkeypatch):
    ...
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.metrics.fetch_slots", lambda port, **kw: []
    )
    win._monitor.collect_monitor_data()
...
def test_router_form_hides_model_level_settings(win):
    ...
    assert not win._configure_panel._widgets["ctx-size"].isVisibleTo(
        win._configure_panel.configure_tab
    )
    assert win._configure_panel._widgets["models-max"].isVisibleTo(
        win._configure_panel.configure_tab
    )
...
def test_server_form_hides_router_only_settings(win):
    win._configure_panel.load_profile(
        Profile(name="Solo", image="img", model="/m.gguf")
    )
    assert not win._configure_panel._widgets["models-max"].isVisibleTo(
        win._configure_panel.configure_tab
    )
    assert win._configure_panel._widgets["ctx-size"].isVisibleTo(
        win._configure_panel.configure_tab
    )
...
def test_relaunch_does_not_force_kill_a_running_router(win, monkeypatch):
    ...
    monkeypatch.setattr(win._launch, "_validate_or_warn", lambda: True)
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.runtime.container_state",
        lambda name, binary, connection="": "running",
    )
    spawned = []
    monkeypatch.setattr(
        win._launch, "_spawn_async", lambda argv, on_done=None: spawned.append(argv)
    )
    # A live headless host may have a 100GB model resident and requests in
    # flight; Launch must not tear it down without asking.
    monkeypatch.setattr(
        "llama_launcher.ui.controllers.launch_controller.QMessageBox.question",
        staticmethod(
            lambda *a, **k: (
                __import__("PySide6.QtWidgets", fromlist=["QMessageBox"]).QMessageBox.No
            )
        ),
    )
    win._launch.on_launch()
...
def test_router_load_failure_is_surfaced(win, monkeypatch):
    _router_win(win)
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.router_api.load_model", lambda *a, **kw: False
    )
...
def test_connected_state_distinguishes_down_from_empty(win, monkeypatch):
    ...
    assert (
        "disconnected"
        in win._configure_panel.configure_status.status_label.text().lower()
    )
    ...
    assert (
        "disconnected"
        not in win._configure_panel.configure_status.status_label.text().lower()
    )
...
def test_failed_router_launch_is_reported(win):
    ...
    win._launch._report_launch_error(
...
def test_member_fields_are_editable(win):
    ...
    win._configure_panel.set_member_fields(
        0, model_id="pinned-id", load_on_startup=True, stop_timeout=30
    )
...
def test_edited_member_fields_survive_a_save_load_round_trip(win):
    ...
    win._configure_panel.set_member_fields(
        0, model_id="pinned", load_on_startup=True, stop_timeout=45
    )
    ...
    assert back.members == [
        RouterMember(
            profile="Qwen", model_id="pinned", load_on_startup=True, stop_timeout=45
        )
    ]
...
def test_spec_counters_feed_the_monitor_in_router_mode(win, monkeypatch):
    ...
    seen = {}
    monkeypatch.setattr(
        "llama_launcher.ui.main_window.metrics.fetch_metrics_text",
...
def test_detached_checkbox_saves_into_profile(win):
    win._configure_panel.load_profile(
        Profile(
            name="Solo", image="img", model="/models/a.gguf", settings={"port": 8080}
        )
    )
...
def test_member_candidates_include_profile_saved_this_session(win):
    ...
    win._configure_panel.mode_combo.setCurrentIndex(
        win._configure_panel.mode_combo.findData("router")
    )
...
def test_stopped_instance_card_offers_remove(win):
    ...
    win.monitor_panel.set_instance_cards(
        {
            "rows": [
                {
                    "name": "llama-dead",
...
def test_router_status_clears_when_router_not_running(win, monkeypatch):
    ...
    win._configure_panel.load_profile(
        Profile(
            name="R", mode="router", image="img", members=[RouterMember(profile="g12b")]
        )
    )
