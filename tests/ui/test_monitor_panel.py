...
def test_set_props_omits_absent_fields(qtbot):
    ...
    panel.set_props(
        PropsInfo(
            build=None, n_ctx=4096, model_alias=None, total_slots=None, modalities={}
        )
    )
    t = panel.info_label.text()
...
def test_set_instance_cards_rebuilds_on_membership_change(qtbot):
    ...
    qtbot.addWidget(panel)
    panel.set_instance_cards({"rows": _rows(), "selected_name": None})
    panel.set_instance_cards(
        {"rows": _rows()[:1], "selected_name": None}
    )  # llama-b gone
...
def test_update_stats_prefers_live_prompt_rate(qtbot):
    ...
    qtbot.addWidget(p)
    p.update_stats(
        {"metrics_on": True, "prompt_tok_s": 12.0, "prompt_tok_s_live": 812.0}
    )
...
def test_update_stats_prompt_falls_back_to_gauge(qtbot):
    ...
    qtbot.addWidget(p)
    p.update_stats(
        {"metrics_on": True, "prompt_tok_s": 12.0, "prompt_tok_s_live": None}
    )
