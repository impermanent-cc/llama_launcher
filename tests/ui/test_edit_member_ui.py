...
def _saved_router_with_member(win):
    ...
    store.save_profile(
        Profile(name="gen", image="img", settings={"port": 8080, "n-gpu-layers": 99}),
        base,
    )
    router = Profile(
        name="myrouter",
...
def test_edit_member_on_clean_router_loads_member_no_prompt(win, monkeypatch):
    ...
    called = {"q": 0}
    monkeypatch.setattr(
        "llama_launcher.ui.panels.configure_panel.QMessageBox.question",
        lambda *a, **k: called.__setitem__("q", called["q"] + 1),
    )
    win._configure_panel._on_edit_member()
    assert called["q"] == 0  # clean => no prompt
    assert (
        win._configure_panel.current_profile().name == "gen"
    )  # form now shows the member
...
def test_edit_member_with_unsaved_changes_prompts_and_cancel_aborts(win, monkeypatch):
    _saved_router_with_member(win)
    win._configure_panel.name_edit.setText(
        "myrouter-edited"
    )  # unsaved change to the router
    ...
    win._configure_panel._on_edit_member()
    assert (
        win._configure_panel.current_profile().name == "myrouter-edited"
    )  # stayed on the router
...
def test_edit_member_missing_profile_warns(win, monkeypatch):
    ...
    router = Profile(
        ...
        image="img",
        settings={"port": 9000},
        members=[RouterMember(profile="ghost")],
    )
    ...
    seen = {}
    monkeypatch.setattr(
        "llama_launcher.ui.panels.configure_panel.QMessageBox.warning",
...
def test_members_guidance_label_present(win):
    ...
    win._configure_panel.load_profile(
        P(name="r", mode="router", image="img", settings={"port": 9000})
    )
    assert win._configure_panel.members_guidance.text().strip()
    assert win._configure_panel.members_guidance.isVisibleTo(
        win._configure_panel.members_guidance.parentWidget()
    )
