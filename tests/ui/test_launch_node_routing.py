...
def _remote_detached_profile():
    return Profile(name="d", image="img:tag", runtime=Runtime(node="box-b", detached=True),
                   mounts=[Mount(host="/h", container="/models", role="model", mode="ro")],
                   model="/models/m.gguf", settings={"port": 8080})


def test_detached_launch_threads_connection_into_rm_argv_cleanup(main_window, monkeypatch):
