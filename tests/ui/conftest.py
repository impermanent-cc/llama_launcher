...
def _hermetic_ui_boundaries(monkeypatch):
    # No real podman spawn from async Stop/Restart in tests. The stub must
    # accept on_error too: callers forward on_error=... explicitly (even when
    ...
    monkeypatch.setattr(
        _runtime, "container_state", lambda name, binary, connection="": "absent"
    )
    ...
    monkeypatch.setattr(
        _runtime, "started_at", lambda name, binary, connection="": None
    )
    monkeypatch.setattr(
        _runtime, "list_local_images", lambda binary, engine="llama.cpp": []
    )
    ...
    monkeypatch.setattr(
        _metrics, "fetch_metrics_text", lambda port, timeout=1.0, **kw: ""
    )
    ...
    monkeypatch.setattr(
        _registry, "fetch_latest", lambda repo, prefix, timeout=10.0: None
    )
    yield
