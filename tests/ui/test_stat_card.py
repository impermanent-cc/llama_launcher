...
def test_stop_button_emits_name(qtbot):
    ...
    qtbot.addWidget(c)
    got = []
    c.stop_requested.connect(got.append)
    c.stop_button().click()
...
def test_stopped_card_action_button_offers_remove(qtbot):
    c = StatCard("llama-dead")
    qtbot.addWidget(c)
    c.update_row(_row(running=False, health="down"))
    assert c.stop_button().text() == "\u2715"
    got = []
    c.remove_requested.connect(got.append)
    stop_got = []
    c.stop_requested.connect(stop_got.append)
    c.stop_button().click()
