...
def test_check_for_update_finds_newer(qtbot):
    ...
    newer = w._launch.check_for_update(
        ["server-cuda12-b9628", "server-cuda12-b9755", "buildcache-x"]
    )
...
def test_check_for_update_none_when_current(qtbot):
    ...
    w._configure_panel.load_profile(
        Profile(
            name="u",
            image="ghcr.io/ggml-org/llama.cpp:server-cuda12-b9755",
            runtime=Runtime(binary="podman"),
            settings={"port": 8080},
        )
    )
    assert (
        w._launch.check_for_update(["server-cuda12-b9628", "server-cuda12-b9755"])
        is None
    )
