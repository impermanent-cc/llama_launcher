...
def test_unmatched_container_has_no_port():
    containers = [
        {"name": "llama-gone", "running": False, "profile": "gone", "mode": "server"}
    ]
...
def test_carries_stop_timeout_from_profile():
    containers = [
        {"name": "llama-slow", "running": True, "profile": "slow", "mode": "server"}
    ]
    profiles = [
        Profile(name="slow", runtime=Runtime(stop_timeout=60), settings={"port": 8080})
    ]
    (inst,) = build_instances(containers, profiles)
...
def test_carries_binary_from_profile():
    containers = [
        {"name": "llama-d", "running": True, "profile": "d", "mode": "server"}
    ]
    profiles = [
        Profile(name="d", runtime=Runtime(binary="docker"), settings={"port": 8080})
    ]
    ...
    profiles = [
        Profile(
            ...
            runtime=Runtime(
                launch_mode="rpc",
                rpc_workers=[
                    RpcWorker(node="box1", device="CPU"),
                    RpcWorker(node="box2", device="CUDA0"),
                ],
            ),
        )
    ]
...
def test_rpc_worker_device_empty_when_index_out_of_range():
    ...
    profiles = [
        Profile(
            name="pool",
            settings={"port": 8080},
            runtime=Runtime(launch_mode="rpc", rpc_workers=[RpcWorker(node="box1")]),
        )
    ]
    (inst,) = build_instances(containers, profiles)
    assert inst.device == ""
