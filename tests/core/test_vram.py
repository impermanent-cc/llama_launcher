...
def test_estimate_for_model_matches_estimate_total_bytes():
    ...
    meta = _meta()
    got = estimate_for_model(meta, 1000, ctx_size=100, k_quant="f16", v_quant="f16")
    direct = estimate(
        n_layers=2,
        n_head=8,
        n_head_kv=4,
...
def test_estimate_for_model_falls_back_to_ctx_train_then_default():
    ...
    direct_train = estimate(
        n_layers=2, n_head=8, n_head_kv=4, n_embd=64, ctx=512, weights_bytes=0
    )
    ...
    direct_default = estimate(
        n_layers=2, n_head=8, n_head_kv=4, n_embd=64, ctx=4096, weights_bytes=0
    )
