...
from llama_launcher.core.command_builder import (
    build_command, needs_server_entrypoint, image_tag,
)


def test_image_tag():
    assert image_tag("ghcr.io/ggml-org/llama.cpp:full") == "full"
    assert image_tag("ghcr.io/ggml-org/llama.cpp:server-cuda12-b9628") == "server-cuda12-b9628"
...
def test_needs_server_entrypoint():
    ...
    assert needs_server_entrypoint("ghcr.io/ggml-org/llama.cpp:server-cuda12-b9628") is False
    assert needs_server_entrypoint("ghcr.io/ggml-org/llama.cpp") is False


def _profile(image, extra=""):
    return Profile(name="p", image=image,
