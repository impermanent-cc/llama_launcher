...
from llama_launcher.services.benchmark import BenchmarkRun, BenchmarkRow


def _run(ts, pp, gen, size=512):
    return BenchmarkRun(ts, [size], 128, 1, 3,
                        [BenchmarkRow(size, size, pp, gen, 1.0)], {"model": "m"})
...
def test_append_caps_at_five(tmp_path):
    ...
    assert len(got) == 5
    assert [r["timestamp"] for r in got] == ["t2", "t3", "t4", "t5", "t6"]  # oldest dropped
