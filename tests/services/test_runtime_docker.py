...
def test_parse_ps_json_accepts_podman_array():
    ...
    assert rows == [
        {"name": "llama-x", "running": True, "profile": "x", "mode": "server"}
    ]
...
def test_parse_ps_json_accepts_docker_ndjson():
    ...
    out = (
        '{"Names":"llama-a","State":"running","Labels":"llama-launcher.profile=a"}\n'
...
def test_stats_reads_podman_keys():
    with patch.object(
        ...
        "_run",
        lambda a: Fake(
            stdout='[{"cpu_percent":"7.0%","mem_usage":"1GiB / 8GiB"}]', rc=0
        ),
    ):
