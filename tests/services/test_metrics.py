...
from llama_launcher.services.metrics import kv_ratio, kv_usage_ratio
...
def test_fetch_metrics_scopes_to_model_without_autoloading(monkeypatch):
    ...
    out = met.fetch_metrics(8080, model="qwen", api_key="sk-x")
...
def test_fetch_metrics_without_model_sends_no_params(monkeypatch):
    ...
    monkeypatch.setattr(met.requests, "get", fake_get)
    met.fetch_metrics(8080)
    assert seen["params"] is None


def test_fetch_metrics_text_returns_raw_body(monkeypatch):
    monkeypatch.setattr(met.requests, "get", lambda *a, **kw: _Resp(text="raw body"))
    assert met.fetch_metrics_text(8080) == "raw body"
...
def test_fetch_metrics_uses_supplied_host(monkeypatch):
    ...
    monkeypatch.setattr(met.requests, "get", fake_get)
    met.fetch_metrics(8080, host="192.168.1.9")
...
def test_fetch_props_none_on_503(monkeypatch):
    monkeypatch.setattr(met.requests, "get", lambda *a, **k: _RespProps(503))
    assert met.fetch_props(8080) is None


def test_fetch_props_none_on_bad_json(monkeypatch):
    monkeypatch.setattr(met.requests, "get", lambda *a, **k: _RespProps(200, None))
    assert met.fetch_props(8080) is None
...
def test_prompt_progress_takes_processing_slot():
    slots = [
        {"is_processing": True, "n_prompt_tokens_processed": 1200},
...
def test_prompt_progress_max_across_processing_slots():
    slots = [
        {"is_processing": True, "n_prompt_tokens_processed": 300},
...
def test_prompt_progress_none_when_nothing_processing():
    ...
    assert (
        met.prompt_progress([{"is_processing": False, "n_prompt_tokens_processed": 50}])
        is None
    )
