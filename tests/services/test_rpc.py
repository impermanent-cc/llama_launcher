...
def test_launch_pool_starts_workers_before_head(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda base, name: {
            ...
            "box2": Node(
                name="box2", kind="remote", connection="box2", ssh_target="user@box2"
            ),
        }.get(name),
    )
    p = Profile(
    ...
    assert res.ok
    head_idx = next(
        i for i, a in enumerate(order) if isinstance(a, list) and "--rpc" in a
    )
    worker_idxs = [
        i
...
def test_stop_pool_stops_head_then_workers(tmp_path, monkeypatch):
    ...
    p = Profile(
        ...
        image="img",
        runtime=Runtime(
            launch_mode="rpc", rpc_workers=[RpcWorker(node="box2", device="CPU")]
        ),
    )
    order = []
...
def test_launch_pool_head_fail_clears_tunnels_entry(tmp_path, monkeypatch):
    ...
    p = Profile(
        ...
        runtime=Runtime(
            launch_mode="rpc",
            rpc_workers=[RpcWorker(node="box2", device="CPU", port=50052)],
        ),
    )
...
def test_launch_pool_terminates_stale_tunnels_on_relaunch(tmp_path, monkeypatch):
    ...
    res1 = rpc.launch_pool(
        p,
        tmp_path,
        run=lambda argv: 0,
        popen=make_popen("first"),
        connect=lambda a: object(),
        alloc_port=lambda: 41000,
    )
    assert res1.ok
    ...
    res2 = rpc.launch_pool(
        p,
        tmp_path,
        run=lambda argv: 0,
        popen=make_popen("second"),
        connect=lambda a: object(),
        alloc_port=lambda: 41001,
    )
    assert res2.ok

    assert all(h.terminated for h in first_handles)  # first call's tunnels not leaked
    assert all(
        h.gen == "second" for h in rpc._TUNNELS["pool"]
    )  # only second call's tunnels live
