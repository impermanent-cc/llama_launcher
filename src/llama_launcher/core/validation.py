...
def _under_any_mount(path: str, profile: Profile) -> bool:
    return any(
        path.startswith(m.container.rstrip("/") + "/") or path == m.container
...
def validate(
    ...
    native_binary_ok: bool = True,
    image_present: bool = True,
    worker_image_present: dict | None = None,
    worker_free_mb: dict | None = None,
) -> list[Issue]:
    ...
    if is_native:
        ...
        if not nb:
            issues.append(
                Issue("error", "Native mode needs a llama-server binary path.")
            )
        elif not native_binary_ok:
            issues.append(
                Issue("error", f"Native binary not found or not executable: {nb}")
            )

    if not is_native and not binary_found:
        issues.append(
            Issue("error", f"Runtime '{profile.runtime.binary}' not found on PATH.")
        )
    ...
    exposed = profile.runtime.bind_host not in LOOPBACK_HOSTS or (
        not is_native and run_args_expose(profile.runtime.extra_run_args)
    )
    if exposed:
    ...
    if profile.mode == "router":
        ...
        for lora in profile.loras:
            if not is_native and not _under_any_mount(lora.path, profile):
                issues.append(
                    Issue("error", f"LoRA path not under any mount: {lora.path}")
                )
    ...
    if profile.mode != "router":
        ...
        if profile.draft_model and profile.settings.get("spec-type", "none") in (
            "none",
            "",
            None,
        ):
        ...
        if profile.runtime.engine == "ik_llama.cpp" and profile.settings.get(
            "run-time-repack"
        ):
            msg = (
                "Run-time repack (--run-time-repack) disables mmap and increases load "
                "time and RAM."
            )
    ...
    if profile.runtime.launch_mode == "rpc":
        issues += _validate_rpc(
            profile, worker_image_present or {}, worker_free_mb or {}
        )
...
def _validate_rpc(
    profile: Profile, worker_image_present: dict, worker_free_mb: dict
) -> list[Issue]:
    issues: list[Issue] = []
    if profile.mode == "router":
...
def _validate_router(
    profile: Profile, members: tuple, api_key_present: bool
) -> list[Issue]:
    issues: list[Issue] = []

    if not members:
        issues.append(
            Issue("error", "A router needs at least one model; add member profiles.")
        )
    ...
    for member, member_profile in members:
        ...
        if not member_profile.model:
            issues.append(
                Issue("error", f"Member '{member_profile.name}' has no model selected.")
            )
    ...
    origins = str(
        profile.settings.get("cors-origins") or CATALOG["cors-origins"].default
    )
    uses_agent_tools = bool(
        profile.settings.get("tools")
    ...
    if (
        profile.runtime.bind_host not in _LOOPBACK_HOSTS
        and origins == "*"
