...
_ALL = [
    ...
    Setting("hipblas", "GGML_HIPBLAS", "bool", False, "Backend", (), engine="ik_llama.cpp",
    ...
    Setting("blas", "GGML_BLAS", "bool", False, "Backend", (), engine="llama.cpp",
    ...
    Setting("blas-vendor", "GGML_BLAS_VENDOR", "string", "Generic", "Backend", (),
            engine="llama.cpp",
    ...
    Setting("opencl", "GGML_OPENCL", "bool", False, "Backend", (), engine="llama.cpp",
            tooltip="Build the OpenCL backend, mainly used for Adreno GPUs on "
                    "Android/Snapdragon devices. Off by default."),
    Setting("sycl", "GGML_SYCL", "bool", False, "Backend", (),
            tooltip="Build the SYCL backend for Intel GPUs via oneAPI. Off by "
                    "default."),
    Setting("sycl-f16", "GGML_SYCL_F16", "bool", False, "Backend", (),
            tooltip="Use 16-bit floats for SYCL calculations, trading a little "
                    "precision for speed on Intel GPUs. Off by default."),
    Setting("sycl-target", "GGML_SYCL_TARGET", "string", "INTEL", "Backend", (),
            tooltip="Target device family for the SYCL backend, e.g. 'INTEL', "
                    "'NVIDIA', 'AMD'. Defaults to 'INTEL'."),
    Setting("musa", "GGML_MUSA", "bool", False, "Backend", (),
            tooltip="Build the MUSA backend for Moore Threads GPUs. Off by "
                    "default."),
    Setting("webgpu", "GGML_WEBGPU", "bool", False, "Backend", (), engine="llama.cpp",
            tooltip="Build the experimental WebGPU backend. Off by default."),
    Setting("cpu-backend", "GGML_CPU", "bool", True, "Backend", (), engine="llama.cpp",
            tooltip="Build the CPU backend. On by default; only turn off for a "
                    "GPU-only build (e.g. an RPC server node with no local "
                    "inference)."),
    Setting("backend-dl", "GGML_BACKEND_DL", "bool", False, "Backend", (),
            engine="llama.cpp",
            tooltip="Build backends as dynamic libraries loaded at runtime instead "
                    ...
                    "default."),
    Setting("cpu-all-variants", "GGML_CPU_ALL_VARIANTS", "bool", False, "Backend", (),
            engine="llama.cpp",
    ...
    # CUDA tuning
    Setting("cuda-fa", "GGML_CUDA_FA", "bool", True, "CUDA tuning", (), engine="llama.cpp",
            tooltip="Compile the FlashAttention CUDA kernels. On by default; "
                    "needed for --flash-attn on CUDA."),
    Setting("cuda-fa-all-quants", "GGML_CUDA_FA_ALL_QUANTS", "bool", False,
            "CUDA tuning", (),
    ...
    Setting("cuda-force-mmq", "GGML_CUDA_FORCE_MMQ", "bool", False, "CUDA tuning", (),
            tooltip="Always use the mmq (matrix-multiply-quantized) CUDA kernels "
                    ...
                    "Off by default."),
    Setting("cuda-force-cublas", "GGML_CUDA_FORCE_CUBLAS", "bool", False,
            "CUDA tuning", (),
            tooltip="Always use cuBLAS instead of the mmq kernels, the opposite of "
                    "cuda-force-mmq. Off by default."),
    Setting("cuda-graphs", "GGML_CUDA_GRAPHS", "bool", True, "CUDA tuning", (),
            engine="llama.cpp",
    ...
    Setting("cuda-no-vmm", "GGML_CUDA_NO_VMM", "bool", False, "CUDA tuning", (),
            tooltip="Disable CUDA virtual memory management even if the driver "
                    "supports it. Off by default (VMM used when available)."),
    Setting("cuda-no-peer-copy", "GGML_CUDA_NO_PEER_COPY", "bool", False,
            "CUDA tuning", (),
    ...
    Setting("cuda-peer-max-batch-size", "GGML_CUDA_PEER_MAX_BATCH_SIZE", "int", 128,
            "CUDA tuning", (), 0, 4096, 1,
    ...
    Setting("cuda-iqk-force-bf16", "GGML_CUDA_IQK_FORCE_BF16", "bool", False,
            ...
            tooltip="Use bf16 cuBLAS when no matching MMQ kernel is available for "
                    ...
                    "Off by default."),
    Setting("cuda-use-graphs", "GGML_CUDA_USE_GRAPHS", "bool", True, "CUDA tuning", (),
            engine="ik_llama.cpp",
            tooltip="ik_llama.cpp's equivalent of cuda-graphs: use CUDA graphs to "
                    "cut per-step launch overhead. On by default."),
    Setting("cuda-fusion", "GGML_CUDA_FUSION", "int", 1, "CUDA tuning", (), 0, 1, 1,
            engine="ik_llama.cpp",
            tooltip="Enable (1, the default) or disable (0) ik_llama.cpp's CUDA "
                    "kernel-fusion optimizations that combine adjacent ops into "
                    "one kernel launch."),
    Setting("cuda-min-batch-offload", "GGML_CUDA_MIN_BATCH_OFFLOAD", "int", 32,
            "CUDA tuning", (), 1, 4096, 1, engine="ik_llama.cpp",
            tooltip="Minimum batch size before a GPU offload is worth doing; "
                    "smaller batches stay on CPU. Default 32."),
    Setting("cuda-dmmv-x", "GGML_CUDA_DMMV_X", "int", 32, "CUDA tuning", (),
            1, 256, 1, engine="ik_llama.cpp",
            tooltip="X stride used by the dmmv (dequantize-matmul-vec) CUDA "
                    "kernels; a low-level kernel-tuning knob. Default 32."),
    Setting("cuda-mmv-y", "GGML_CUDA_MMV_Y", "int", 1, "CUDA tuning", (),
            1, 16, 1, engine="ik_llama.cpp",
            tooltip="Y block size used by the mmv (matmul-vec) CUDA kernels; a "
                    "low-level kernel-tuning knob. Default 1."),
    Setting("cuda-kquants-iter", "GGML_CUDA_KQUANTS_ITER", "int", 2, "CUDA tuning", (),
            1, 8, 1, engine="ik_llama.cpp",
            tooltip="Iterations per thread-block for the Q2_K/Q6_K CUDA dequant "
                    "kernels; a low-level kernel-tuning knob. Default 2."),
    Setting("cuda-force-dmmv", "GGML_CUDA_FORCE_DMMV", "bool", False, "CUDA tuning", (),
            engine="ik_llama.cpp",
            tooltip="Always use the dmmv CUDA kernels instead of mmvq. Off by "
                    "default."),

    # CPU & ISA
    Setting("native-opt", "GGML_NATIVE", "bool", True, "CPU & ISA", (),
    ...
    Setting("lto", "GGML_LTO", "bool", False, "CPU & ISA", (),
    ...
    Setting("cpu-repack", "GGML_CPU_REPACK", "bool", True, "CPU & ISA", (),
            ...
            tooltip="Repack Q4_0 weights into runtime-optimized Q4_X_X layouts "
                    ...
                    "option.)"),
    Setting("cpu-hbm", "GGML_CPU_HBM", "bool", False, "CPU & ISA", (),
    ...
    Setting("iqk-flash-attention", "GGML_IQK_FLASH_ATTENTION", "bool", True,
            "ik kernels", (), engine="ik_llama.cpp",
            tooltip="Compile ik_llama.cpp's CPU FlashAttention kernels. On by "
                    "default."),
    Setting("iqk-fa-all-quants", "GGML_IQK_FA_ALL_QUANTS", "bool", True,
            "ik kernels", (), engine="ik_llama.cpp",
    ...
    Setting("expert-chunking", "GGML_EXPERT_CHUNKING", "bool", True, "ik kernels", (),
            engine="ik_llama.cpp",
            tooltip="Process MoE experts in chunks to reduce peak memory during "
                    "inference. On by default."),

    # Vulkan tuning
    Setting("vulkan-no-coopmat", "GGML_VULKAN_NO_COOPMAT", "bool", False,
            "Vulkan tuning", (), engine="ik_llama.cpp",
    ...
    Setting("vulkan-no-coopmat2", "GGML_VULKAN_NO_COOPMAT2", "bool", False,
            "Vulkan tuning", (), engine="ik_llama.cpp",
            tooltip="Don't use the Vulkan cooperative-matrix2 extension even if "
                    "supported. Off by default."),
    Setting("vulkan-no-int-dot", "GGML_VULKAN_NO_INT_DOT", "bool", False,
            "Vulkan tuning", (), engine="ik_llama.cpp",
            tooltip="Don't use Vulkan's integer dot-product extension even if "
                    "supported. Off by default."),
    Setting("vulkan-no-bf16", "GGML_VULKAN_NO_BF16", "bool", False,
            "Vulkan tuning", (), engine="ik_llama.cpp",
            tooltip="Don't use Vulkan bf16 support even if the driver has it. "
                    "Off by default."),

    # Features & networking
    Setting("rpc", "GGML_RPC", "bool", False, "Features & networking", (),
            tooltip="Build RPC support so this build can act as (or connect to) a "
                    ...
                    "by default."),
    Setting("curl", "LLAMA_CURL", "bool", False, "Features & networking", (),
            engine="ik_llama.cpp",
    ...
    Setting("build-server", "LLAMA_BUILD_SERVER", "bool", True,
            "Features & networking", (),
            tooltip="Build the llama-server example (what this app launches). On "
                    "by default for a standalone build; turn off only for a "
                    "library-only build."),
    Setting("build-mtmd", "LLAMA_BUILD_MTMD", "bool", False,
            "Features & networking", (), engine="llama.cpp",
    ...
    Setting("build-tools", "LLAMA_BUILD_TOOLS", "bool", True,
            "Features & networking", (), engine="llama.cpp",
            tooltip="Build llama.cpp's CLI tools (quantize, convert helpers, "
                    "etc.), not just the server library. On by default for a "
                    "standalone build."),
    Setting("build-examples", "LLAMA_BUILD_EXAMPLES", "bool", True,
            "Features & networking", (),
            tooltip="Build the example programs. On by default for a standalone "
                    "build."),
    Setting("build-tests", "LLAMA_BUILD_TESTS", "bool", True,
            "Features & networking", (),
    ...
    # Build type & misc
    Setting("build-type", "CMAKE_BUILD_TYPE", "enum", "Release",
            "Build type & misc", (),
    ...
    Setting("static", "GGML_STATIC", "bool", False, "Build type & misc", (),
            tooltip="Statically link the runtime libraries into the binary. Off "
                    ...
                    "a larger file."),
    Setting("ccache", "GGML_CCACHE", "bool", True, "Build type & misc", (),
