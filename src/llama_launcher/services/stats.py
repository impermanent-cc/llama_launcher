...
def build_snapshot(
    container_name: str, binary: str, cpu_sampler: CpuSampler
) -> StatsSnapshot:
    gpus = query_gpus()
    ...
    if stat_text is not None:
        ...
        cpu = CpuStat(
            overall_pct=overall, per_core_pct=cores, load=parse_loadavg(load_text)
        )
        mem = parse_meminfo(mem_text)
    else:
        cpu, mem = None, None
    container = (
        query_container_stats(container_name, binary) if container_name else None
    )
    return StatsSnapshot(
        gpus=gpus, cpu=cpu, mem=mem, container=container, gpu_available=bool(gpus)
    )
