...
from llama_launcher.core.nodes import Node, connection_for, valid_ssh_target
from llama_launcher.services import gpu, runtime
from llama_launcher.store.nodes import add_node, get_node, load_nodes, remove_node
...
def _run_ok(argv: list[str]) -> bool:
    ...
    try:
        return (
            subprocess.run(argv, capture_output=True, text=True, timeout=15).returncode
            == 0
        )
...
class NodesDialog(QDialog):
    def __init__(self, base_dir: Path, parent=None):
        ...
        self.ssh_edit = QLineEdit()
        self.ssh_edit.setPlaceholderText("user@host[:port]")
    ...
    def add_node_from_fields(self, name: str, ssh_target: str, binary: str) -> None:
        ...
        if name == "local":
            QMessageBox.warning(
                self, "Nodes", '"local" is reserved for the local node.'
            )
            return
        ...
        if not valid_ssh_target(ssh_target):
            QMessageBox.warning(self, "Nodes", f"Invalid SSH target {ssh_target!r}.")
            return
        ...
        if not _run_ok(runtime.connection_add_argv(name, ssh_target, binary)):
            QMessageBox.warning(
                self, "Nodes", f"Could not add {binary} connection for {name!r}."
            )
            return
        add_node(
            Node(
    ...
    def test_node(
        self, connection: str, ssh_target: str, binary: str = "podman"
    ) -> tuple[bool, bool]:
